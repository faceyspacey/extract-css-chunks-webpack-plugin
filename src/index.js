...
class ExtractCssChunks {
    ...
    apply(compiler) {
        try {
            ...
            throw new Error(
                `Something went wrong: contact the author: ${JSON.stringify(e)}`
            );
        }
            ...
            compilation.hooks.normalModuleLoader.tap(pluginName, (lc, m) => {
                ...
                loaderContext[MODULE_TYPE] = (content) => {
                    ...
                    for (const line of content) {
                        const count =
                            identifierCountMap.get(line.identifier) || 0;
                        module.addDependency(
                            new CssDependency(line, m.context, count)
                        );
            ...
            compilation.mainTemplate.hooks.hashForChunk.tap(
                ...
                (hash, chunk) => {
                    ...
                    if (REGEXP_CHUNKHASH.test(chunkFilename)) {
                        hash.update(
                            JSON.stringify(chunk.getChunkMaps(true).hash)
                        );
                    }
                    ...
                    if (REGEXP_NAME.test(chunkFilename)) {
                        hash.update(
                            JSON.stringify(chunk.getChunkMaps(true).name)
                        );
                    }
                }
            );
                        ...
                        const linkHrefPath = mainTemplate.getAssetPath(
                            JSON.stringify(this.options.chunkFilename),
                            {
                                ...
                                chunk: {
                                    ...
                                    hashWithLength(length) {
                                        const shortChunkHashMap = Object.create(
                                            null
                                        );
                                        for (const chunkId of Object.keys(
                                            chunkMaps.hash
                                        )) {
                                            if (
                                                typeof chunkMaps.hash[
                                                    chunkId
                                                ] === 'string'
                                            ) {
                                                shortChunkHashMap[
        ...
        if (typeof chunkGroup.getModuleIndex2 === 'function') {
            ...
            while (usedModules.size < modules.length) {
                ...
                for (const list of modulesByChunkGroup) {
                    ...
                    while (
                        list.length > 0 &&
                        usedModules.has(list[list.length - 1])
                    ) {
                        list.pop();
                    }
                    ...
                    if (list.length !== 0) {
                        ...
                        const failedDeps = Array.from(deps).filter(
                            unusedModulesFilter
                        );
                ...
                if (!success) {
                    ...
                    compilation.warnings.push(
                        new Error(
                            `chunk ${chunk.name ||
                                chunk.id} [extract-css-chunks-plugin]\n` +
